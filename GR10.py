...
class Grid():
    ...
    def reset(self):
        padded_size = self.grid_size + 2 * 1
        ...
        # Edges
        self.grid[0:1, :] = EDGE_VALUE
        self.grid[-1*1:, :] = EDGE_VALUE
        self.grid[:, 0:1] = EDGE_VALUE
        self.grid[:, -1*1:] = EDGE_VALUE

        # Map
        for _, edge_point in enumerate(self.edge_list):
            self.grid[edge_point[0]+1, edge_point[1]+1] = EDGE_VALUE


        for _, goal_point in enumerate(self.goal_list):
            self.grid[goal_point[0]+1, goal_point[1]+1] = GOAL_VALUE
    ...
    def update(self, update):
        ...
        if update == True:
            for index, goal in enumerate(self.goal_list):
                if self.grid[goal[0]+1, goal[1]+1] == 0:
                    goal_bk.pop(index)
                    self.rout_list.append(goal)
                    self.get_list.append(goal)
                elif self.grid[goal[0]+1, goal[1]+1] > 0:
                    self.grid[goal[0]+1,goal[1]+1] += STEP_WASTE
                else:
                    continue
            ...
            if len(self.goal_list) <= 10:
                for time in range(random.randint(0,3)):
                    ...
                    self.grid[new_goal[0]+1, new_goal[1]+1] = GOAL_VALUE
    ...
    def env_state(self):
        # print(self.grid[:,:])
        # return self.grid[1:1+10, 1:1+10]
        return self.grid[:,:]
...
class Environment:
    ...
    def record_step(self):
        ...
        self.history.append((grid, self.agent.health, self.grid.goal_get, self.agent.pos))
    ...
    def visible_state(self):
        ...
        y, x = self.agent.pos
        yp = (y - 1) / self.grid.grid_size
        xp = (x - 1) / self.grid.grid_size
    ...
    def step(self, action, step_count, update=False):
        ...
        #     if self.agent.pos == self.history[-2][3]:
        #         value -= 1
...
def select_action(e, state):
    ...
    # action = [0.25, 0.25, 0.25, 0.25]   ## only for random choice
